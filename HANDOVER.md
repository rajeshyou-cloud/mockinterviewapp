...
The proposed replacement or supplement for manual content review is documented in `docs/BENCHMARK_SCORING_PLAN.md`. The reusable automated workflow is documented in `docs/BENCHMARK_REVIEW_AUTOMATION.md` and wrapped as the project-local skill `.codex/skills/benchmark-review-automation/SKILL.md`. The benchmark-answer schema, baseline data migration, evidence-packet export, dual-model review runner, review-status importer, publication gate, dimension scoring, standalone validator, CI gate, and Neon scoring-run migration are implemented. The review runner supports Vercel AI Gateway, direct Anthropic/Claude, and direct ChatGPT/OpenAI execution. To use ChatGPT/OpenAI for automated review, set `OPENAI_API_KEY`, `REVIEW_PROVIDER=openai`, `REVIEW_PRIMARY_MODEL`, and `REVIEW_CRITIC_MODEL`, then run a dry-run such as `npm run review:benchmarks:auto -- --provider=openai --technology=aws --limit=2 --dry-run` before running a full technology pack. Claude web review batches covering Informatica, AWS, Databricks, and Oracle were imported, including Informatica and Oracle re-reviews: 1,350 valid review records processed, 334 currently ai-evidence-verified, and 266 still disputed. Informatica's original 125 disputed records were remediated; second-pass Claude review approved 112 and disputed 38 advanced `-06`/`-07` variants; those remaining 38 were rewritten and passed final Claude re-review. Informatica is now fully ai-evidence-verified at 150/150. Oracle's concept gaps are fixed and all 25 conceptual questions are verified. Claude initially rejected 125 non-conceptual Oracle answers for self-referential template corruption; those were regenerated with clean feature labels, and the next Oracle re-review approved 125 while disputing only the 25 `-06` recovery variants. Those 25 were rewritten to cover evidence preservation, last-known-correct state, reconciliation, idempotent repair, validation, and recurrence prevention; final Claude re-review approved all 150 Oracle records. Oracle is now fully ai-evidence-verified at 150/150. The stale Informatica first-pass, Informatica second-pass, Oracle first-pass, and Oracle second-pass review outputs were moved under `apps/web/data/benchmark-reviews/archive`; the final Oracle approval files are retained under `apps/web/data/benchmark-reviews/oracle-sentforrereview-approved-part-01.reviewed.jsonl` through `part-06`. Still pending: reviewing Snowflake, Power BI, and Python; remediating AWS/Databricks disputed benchmarks; importing real review verdicts into benchmark statuses; withholding disputed or stale questions; and calibration.
